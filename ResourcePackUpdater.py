...
mcmeta = glob.glob('**/textures/**/*.png.mcmeta', recursive=True)
...
# Store paths to JSON files in list
blockstates = glob.glob('**/blockstates/**/*.json', recursive=True)
...
for blockstate in json:
	...
	if variants:
		variants = variants[0]
		model_ref = [v for k,v in variants.items()][0]
		for dic in model_ref:
			model_name = [v for k,v in dic.items() if k == "model"][0]
			if model_name in changes_keys:
				dic["model"] = changes[model_name]
